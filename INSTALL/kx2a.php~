...
The purpose of this script is to quickly and easily run the commands contained within the Anonsaba SQL file, and is
...
<form action="kx2a.php" method="post">
<br />
...
<?php
	...
	echo 'Locating \'kx2a.sql\'... ';
	if (file_exists('kx2a.sql') && (filesize('kx2a.sql') > 0)) {
	echo 'found.<br />';
	$sqlfile = fopen('kx2a.sql', 'r');
	echo 'File opened.<br />';
	$readdata = fread($sqlfile, filesize('kx2a.sql'));
	$readdata = str_replace('PREFIX',KU_DBPREFIX,$readdata);
	fclose($sqlfile);
	echo 'Contents read.<br />';
	}else{
	echo '<font color=red>Error.</font> ';
	die('An error occured. kx2a.sql does not exist in this directory or it is 0 bytes big :( Barring that, do you have read permissions for the directory?');
	}
	...
	foreach ($sqlarray as $sqldata) {
		...
		if (strlen($sqldata) !== 0) { // As the array was exploded on ';', the last ';' caused a blank element to be created as there was no data after it :p
			...
			echo "Attempting to create table '$tablename'...<br />success.<br /> ";
			$tc_db->Execute($sqldata);
	...
	// All done :)
	// Now we will insert all the boardnames in `boardnames` coloumn in posts
	$query1 = $tc_db->GetAll('SELECT `id` FROM `'.KU_DBPREFIX.'boards`');
	foreach ($query1 as $line) {
		$query2 = $tc_db->GetOne('SELECT `name` FROM `'.KU_DBPREFIX.'boards` WHERE `id` = '.$line['id'].'');
		$tc_db->Execute('UPDATE `'.KU_DBPREFIX.'posts` SET `boardname` = "'.$query2.'" WHERE `boardid` = '.$line['id'].'');
	}
	$filename = '.installed';
	$installed = fopen($filename, 'w') or die('Failed to create the .installed file. Please create this manually.');
