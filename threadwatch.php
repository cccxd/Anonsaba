...
if (isset($_GET['do'])) {
	switch($_GET['do']) {
	case 'addthread':
		$threadexist = $tc_db->GetOne('SELECT COUNT(*) FROM `'.KU_DBPREFIX.'posts` WHERE `id` = '.$tc_db->qstr($_GET['threadid']).' AND `boardid` = '.$tc_db->qstr($_GET['board']));
		...
		if ($viewing_thread_is_watched == 0 && $threadexist != 0) {
