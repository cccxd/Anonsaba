...
class Posting {
	...
	function CheckReplyTime() {
		...
		$results = array();
		$result = $tc_db->GetAll("SELECT MAX(timestamp) FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $board_class->board['id'] . " AND `parentid` != 0 AND `ipmd5` = '" . md5($_SERVER['REMOTE_ADDR']) . "' AND `timestamp` > " . (time() - KU_REPLYDELAY));
		foreach ($result as $line) {
			$results = $line['MAX(timestamp)'];
		}
		/* If they have posted before and it was recorded... */
		if (isset($results)) {
		/* If the time was shorter than the minimum time distance */
			if (time() - $results <= KU_REPLYDELAY) {
	...
	function CheckCaptcha() {
		global $tc_db, $board_class;
		if ($board_class->board['enablecaptcha'] == 1 && !$this->CheckIsReply()) {
			if (!$tc_db->GetOne('SELECT `token` FROM `botcheckwhitelist` WHERE `token` = "'.$_POST['captoken'].'"')) {
				require_once('recaptchalib.php');
				$privatekey = "6LdKr9ASAAAAAETmGohRZ7TACJRdCc5PFgikCTTM";
				...
				setcookie('token', urldecode($_POST['captoken']), time() + 31556926, '/', KU_DOMAIN);
			}
		}	
	}
	...
	function CheckBannedHash() {
		...
		if (isset($_FILES['imagefile']) && isset($_FILES['imagefile']['name'])) {
			for($i=0;$i<$board_class->board['max_files'];$i++){
				if (isset($_FILES['imagefile']['name'][$i]) && $_FILES['imagefile']['name'][$i] != '') {
					$results = $tc_db->GetAll("SELECT `bantime` , `description` FROM `" . KU_DBPREFIX . "bannedhashes` WHERE `md5` = " . $tc_db->qstr(md5_file($_FILES['imagefile']['tmp_name'][$i])) . " LIMIT 1");
					if (count($results) > 0) {
						$bans_class->BanUser($_SERVER['REMOTE_ADDR'], 'SERVER', 1, $results[0]['bantime'], '', 'Posting a banned file.<br />' . $results[0]['description'], '', 0, 0, 1, true);
	...
	function GetUserAuthority() {
		...
		if (isset($_POST['modpassword'])) {
			$results = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "staff` WHERE `username` = '" . md5_decrypt($_POST['modpassword'], KU_RANDOMSEED) . "'");
			if (!empty($results)) {
				foreach ($results as $line) {
					if ($line['type'] == 1) {
						$user_authority = 1; // admin
					} elseif ($line['type'] == 2 && in_array($board_class->board['name'], explode('|', $line['boards'])) && $line['access'] == 9) {
						$user_authority = 4; // super mod 
					} elseif ($line['type'] == 2 && $line['boards'] == 'allboards' && $line['access'] == 9) {
						$user_authority = 4;
					} elseif ($line['type'] == 2 && in_array($board_class->board['name'], explode('|', $line['boards'])) && $line['access'] < 9) {
				...
				if ($user_authority < 5 && $user_authority != 3) { /* set posting flags for mods and admins */
	...
	function CheckBadUnicode($post_name, $post_email, $post_subject, $post_message) {
		...
		$ords_filenames = array();
		if (isset($_FILES['imagefile']) && isset($_FILES['imagefile']['name'])) {
			foreach ($_FILES['imagefile']['name'] as $name) {
				$ords_filenames[] = unistr_to_ords($name);
			}
		} else {
			$ords_filenames[] = '';
		}
		foreach ($bad_ords as $bad_ord) {
			...
			foreach ($ords_filenames as $filename) {
				if ($filename != '') {
					if (in_array($bad_ord, $filename)) {
						exitWithErrorPage(_gettext('Your post contains one or more illegal characters.'));
					}
				}
			}
	...
	function CheckBlacklistedText() {
		...
		foreach ($badlinks as $badlink) {
			if (stripos($_POST['message'], $badlink) !== false || stripos($_POST['subject'], $badlink) !== false || stripos($_POST['name'], $badlink) !== false) {
