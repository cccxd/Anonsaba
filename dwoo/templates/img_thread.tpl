...
{/if}
	{foreach key=postkey item=post from=$posts name=postsloop}
		...
		<div id="thread{$post.id}{$board.name}">
			...
			<br />
		{else}
		{if $numimages > 1 && $isexpand && $.foreach.postsloop.first}
	...
	{if not $isread}
		{if $replycount > 2}
			<span style="float:right">
				&#91;<a href="{%KU_BOARDSPATH}/{$board.name}/">{t}Return{/t}</a>&#93;
				{if %KU_FIRSTLAST && ( count($posts) > 50 || $replycount > 50)}
					&#91;<a href="{%KU_BOARDSPATH}/{$board.name}/res/{$posts.0.id}.html">{t}Entire Thread{/t}</a>&#93; 
					&#91;<a href="{%KU_BOARDSPATH}/{$board.name}/res/{$posts.0.id}+50.html">{t}Last 50 posts{/t}</a>&#93;
					{if ( count($posts) > 100 || $replycount > 100) }
						&#91;<a href="{%KU_BOARDSPATH}/{$board.name}/res/{$posts.0.id}-100.html">{t}First 100 posts{/t}</a>&#93;
					{/if}
				{/if}
			</span>
		{/if}
