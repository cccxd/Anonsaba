...
class Bans {
	...
	function RemoveExpiredHashBans() {
		global $tc_db;
		$check = $tc_db->GetAll('SELECT * FROM `'.KU_DBPREFIX.'bannedhashes`');
		foreach ($check as $line) {
			$bannedhashtime = $line['dateadded'] + $line['bantime'];
		}
		$results = $tc_db->Execute('DELETE FROM `'.KU_DBPREFIX.'bannedhashes` WHERE `bantime` != 0 AND `bantime` < '.$bannedhashtime);
	}
