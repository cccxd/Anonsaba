						...
						{if count($post.file) gt 1}
							{foreach key=fileskey name=filesloop item=file from=$post.file}
								...
								{if $fileskey % 3 eq 0}<br style="clear:both" />{/if}
									...
									<span class="{if $.foreach.filesloop.first || $fileskey % 3 eq 0}multithumbfirst{else}multithumb{/if}"><a href="{$file_path}/src/{$post.file.$fileskey}.{$post.file_type.$fileskey}" onclick="javascript:expandimg('{$post.id}-{$fileskey}', '{$file_path}/src/{$post.file.$fileskey}.{$post.file_type.$fileskey}', '{$file_path}/thumb/{$post.file.$fileskey}s.{$post.file_type.$fileskey}', '{$post.image_w.$fileskey}', '{$post.image_h.$fileskey}', '{$post.thumb_w.$fileskey}', '{$post.thumb_h.$fileskey}');return false;">({if $post.image_w.$fileskey > 0 && $post.image_h.$fileskey > 0}{$post.image_w.$fileskey}x{$post.image_h.$fileskey}{else}{$post.file_size_formatted.$fileskey}{/if})</a></span><br />
								...
								{if $fileskey eq 3}<br style="clear:left" />{/if}
									<div style="float:left">
									<span class="{if $.foreach.filesloop.first || $fileskey % 3 eq 0}multithumbfirst{else}multithumb{/if}"><a {if %KU_NEWWINOW}target="_blank" {/if}href="{$file_path}/src/{$post.file.$fileskey}.{$post.file_type.$fileskey}">{$post.file_type.$fileskey|upper} ({$post.file_size_formatted.$fileskey})</a></span><br />
									<a {if %KU_NEWWINDOW}target="_blank" {/if}href="{$file_path}/src/{$post.file.$fileskey}.{$post.file_type.$fileskey}">
									<span id="thumb{$post.id}-{$fileskey}"><img class="{if $.foreach.filesloop.first || $fileskey % 3 eq 0}multithumbfirst{else}multithumb{/if}" src="{$post.nonstandard_file.$fileskey}" alt="{$post.id}" height="{$post.thumb_h.$fileskey}" width="{$post.thumb_w.$fileskey}" title="{if isset($post.id3.$fileskey.comments_html)}{if $post.id3.$fileskey.comments_html.artist.0 neq ''}{$post.id3.$fileskey.comments_html.artist.0}{if $post.id3.$fileskey.comments_html.title.0 neq ''} - {/if}{/if}{if $post.id3.$fileskey.comments_html.title.0 neq ''}{$post.id3.$fileskey.comments_html.title.0}{/if}{else}{$post.file.$fileskey}.{$post.file_type.$fileskey}{/if} - ({$post.file_size_formatted.$fileskey}{if $post.id3.$fileskey.comments_html.bitrate neq 0 || $post.id3.$fileskey.audio.sample_rate neq 0}{if $post.id3.$fileskey.audio.bitrate neq 0} - {round($post.id3.$fileskey.audio.bitrate / 1000)} kbps{if $post.id3.$fileskey.audio.sample_rate neq 0} - {/if}{/if}{if $post.id3.$fileskey.audio.sample_rate neq 0}{$post.id3.$fileskey.audio.sample_rate / 1000} kHz{/if}{/if}{if $post.file_original.$fileskey neq '' && $post.file_original.$fileskey neq $post.file.$fileskey}, {$post.file_original.$fileskey}.{$post.file_type.$fileskey}{/if}){if $post.id3.$fileskey.playtime_string neq ''} {t "Length"}: {$post.id3.$fileskey.playtime_string}{/if}" /></span>
									</a></div>
								{/if}
