{if count($posts) > 0}
	...
	{foreach key=postkey item=post from=$posts}
		...
		<td align="center">
			[<a href="{%KU_BOARDSFOLDER}{$board.name}/src/{$post.file.0}.{$post.file_type.0}" target="_blank">{$post.file.0}.{$post.file_type.0}</a>]
		...
		<td align="center">
			[<a href="{%KU_BOARDSPATH}/{$board.name}/res/{$post.id}.html">Reply</a>]
		</td>
	</tr>
	{/foreach}
	</table></center><br /><hr />
