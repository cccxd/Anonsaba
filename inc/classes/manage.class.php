...
class Manage {
	...
	function ValidateSession($is_menu = false) {
		global $tc_db, $tpl_page;
			if (isset($_SESSION['manageusername']) && isset($_SESSION['managepassword'])) {
                        $results = $tc_db->GetAll("SELECT HIGH_PRIORITY `username` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_SESSION['manageusername']) . " AND `password` = " . $tc_db->qstr($_SESSION['managepassword']) . " AND `suspended` != 1 LIMIT 1");
			if (count($results) == 0) {
				session_destroy();
	...
	function CheckLogin() {
		...
		if (count($results) > 5) {
			...
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY `username`, `password`, `salt` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_POST['username']) . " AND `type` != 3 AND `suspended` != 1 LIMIT 1");
			if ($tc_db->GetOne("SELECT `username` FROM `" . KU_DBPREFIX . "staff` WHERE `suspended` = 1 AND `username` = ".$tc_db->qstr($_POST['username'])."")) {
				exitWithErrorPage(_gettext('You are currently suspended'));
			}
			if (count($results) > 0) {
				if (empty($results[0]['salt'])) {
					if (md5($_POST['password']) == $results[0]['password']) {
						...
						$_SESSION['manageusername'] = $_POST['username'];
						management_addlogentry(_gettext('Failed login attempt (') . $_SERVER['REMOTE_ADDR'] . ')', 1);
						session_destroy();
	...
	function SetModerationCookies() {
		...
		if (isset($_SESSION['manageusername'])) {
			$results = $tc_db->GetOne("SELECT HIGH_PRIORITY `boards` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_SESSION['manageusername']) . " LIMIT 1");
			if ($this->CurrentUserIsAdministrator() || $results == 'allboards') {
				setcookie("kumod", "allboards", time() + 3600, KU_BOARDSFOLDER, KU_DOMAIN);
			} else {
				if ($results != '') {
					setcookie("kumod", $results, time() + 3600, KU_BOARDSFOLDER, KU_DOMAIN);
				}
			}
		}
	}
...
function pms() {
		...
		$tpl_page .='
			...
			if (count($results) > 0) {
				$tpl_page .= '<table border="1" width="100%"><tr><th>'. _gettext('Message') .'</th><th>'. _gettext('From') .'</th><th>'. _gettext('Options') .'</th></tr>';
				foreach ($results as $line) {
					$tpl_page .= '<tr><td>'. $line['message'] . '</td><td>'. $line['from'] . '</td><td>[<a href="?action=pms&amp;del='. $line['id'] . '">'. _gettext('Delete') .'</a>] [<a href="?action=pms&amp;dismiss='. $line['id'] . '">'. _gettext('Dismiss') .'</a>] [<a href="?action=pms&amp;reply='. $line['id'] . '">'._gettext('Reply').'</a>]</td></tr>';
				}
				$tpl_page .= '</table>';
			}
			if ($count == 1) {
				$tpl_page .= _gettext('You currently have <b>1</b> unread message.');
			} else {
				$tpl_page .= _gettext('You currently have <b>' .$count. '</b> unread messages.');
			}
	}
	function checkversion() {
		global $tc_db, $tpl_page;
		$this->AdministratorsOnly();
		$update = file_get_contents("http://anonsaba.net/version.php");
		$version = $tc_db->GetOne("SELECT `version` FROM `".KU_DBPREFIX."options`");
		$tpl_page .= '<h2>'. ('Check for Updates') . '</h2><br />';
		if ($update != $version) {
			$tpl_page .= _gettext('<font color="red">Update Available!</font>');
			$tpl_page .= '<br /><br />'. ('There is currently an update available! Click <a href="http://www.anonsaba.net/" target="_new">here</a> to view the update!');		
		} else {
			$tpl_page .= _gettext('No new updates are available.');
		}
		$tpl_page .="</table>";
	}
	/* Emailing for post/reports */
	function email() {
		global $tc_db, $tpl_page;
		$this->AdministratorsOnly();
		$tpl_page .= '<h2>'. ('Email post and reports') .'</h2><br />';
		$query = $tc_db->GetAll('SELECT * FROM `'.KU_DBPREFIX.'email` ORDER BY `staffname`');
		...
		if (empty($email)) {
			$tpl_page .= '<hr /><h3>'. _gettext('Please add an email to use this feature') .'</h3><hr /><br />';
		}
		if (isset($_POST['submit'])) {
			...
			if (isset($_POST['reportemail'])) {
				$email2 = 1;
			} else {
				$email2 = 0;
			}
			if (!in_array($_POST['staffname'], $staff)) {
				exitWithErrorPage(_gettext('There is no username by that name. Please enter a valid username for '.$_POST['email'].'.'));
			}
			$tc_db->Execute('INSERT HIGH_PRIORITY INTO `' . KU_DBPREFIX . 'email` (`email`, `staffname`, `postemail`, `reportemail`) VALUES ("'.$_POST['email'].'", "'.$_POST['staffname'].'", '.$email1.', '.$email2.')');
			$tpl_page .= '<hr /><h3>'. _gettext($_POST['email'].' has been added.').'</h3><hr /><br />';
		}
		if ($_GET['del']) {
			$tc_db->Execute("DELETE FROM `" . KU_DBPREFIX . "email` WHERE `id` = " . $tc_db->qstr($_GET['del']) . "");
			$tpl_page .= '<hr /><h3>'. _gettext('Email has been deleted.') .'</h3><hr />';
		}
		if ($_GET['edit']) {
			if (isset($_POST['add'])) {
				if (isset($_POST['postemail'])) {
				...
				$tc_db->Execute('UPDATE `'.KU_DBPREFIX.'email` SET `email` = "'.$_POST['emailtxt'].'", `staffname` = "'.$_POST['staffname'].'", `postemail` = '.$postemail1.', `reportemail` = '.$reportemail1.' WHERE `id` = '.$tc_db->qstr($_GET['edit']));
				$tpl_page .= '<hr /><h3>'. _gettext('Email succesfully edited.') .'</h3><hr />';
			}
			$check = $tc_db->GetOne('SELECT `postemail` FROM `'.KU_DBPREFIX.'email` WHERE `id` = '.$tc_db->qstr($_GET['edit']));
			$check1 = $tc_db->GetOne('SELECT `reportemail` FROM `'.KU_DBPREFIX.'email` WHERE `id` = '.$tc_db->qstr($_GET['edit']));
			if ($check == 1) {
			...
			$id = $tc_db->GetOne('SELECT `id` FROM `'.KU_DBPREFIX.'email` WHERE `id` = '.$tc_db->qstr($_GET['edit']));
			$tpl_page .='<form action="manage_page.php?action=email&amp;edit='.$id.'" method="post">
						<label for="emailtxt">' ._gettext('Email').':</label>
						<input type ="text" id="emailtxt" name="emailtxt" value="'.$tc_db->GetOne('SELECT `email` FROM `'.KU_DBPREFIX.'email` WHERE `id` = '.$tc_db->qstr($_GET['edit'])).'" /><br />
						<label for="staffname">'. _gettext('Staff Username') .':</label>
						<input type="text" id="staffname" name="staffname" value="'.$tc_db->GetOne('SELECT `staffname` FROM `'.KU_DBPREFIX.'email` WHERE `id` = '.$tc_db->qstr($_GET['edit'])).'" />
						<div class="desc"><b>'. $final . '</b></div><br />
						<label for="postemail">'. _gettext('Send Email for every post') .':</label>
						<input type="checkbox" name="postemail" id="postemail" '.$check.' /><br />
						<label for="reportemail">'. _gettext('Send Email for every report') .':</label>
						<input type="checkbox" name="reportemail" id="reportemail" '.$check1.' />
						<br />
						<input type="submit" name="add" id="add" value="'. _gettext('Submit') . '" />
						</form>';
		} else {
			$tpl_page .= '<form action="manage_page.php?action=email" method="post">
						...
						<div class="desc"><b>'. $final . '</b></div><br />
						<label for="postemail">'. _gettext('Send Email for every post') .':</label>
						<input type="checkbox" name="postemail" id="postemail" /><br />
						<label for="reportemail">'. _gettext('Send Email for every report') .':</label>
						<input type="checkbox" name="reportemail" id="reportemail" />
						<br />
						<input type="submit" name="submit" id="submit" value="'. _gettext('Submit') . '" />
						</form>';
		}
		$results = $tc_db->GetAll("SELECT HIGH_PRIORITY * FROM `" . KU_DBPREFIX . "email` ORDER BY `staffname`");
			if (count($results) > 0) {
				$tpl_page .= '<table border="1" width="100%"><tr><th>'. _gettext('Email') .'</th><th>'. _gettext('Staff Name') .'</th><th>'. _gettext('Email Posts') .'</th><th>'. _gettext('Email Reports') .'</th><th>'. _gettext('Options') .'</th></tr>';
				foreach ($results as $line) {
					if ($line['postemail'] == 1) {
						$answer = 'Yes';
					} else {
					...
					if ($line['reportemail'] == 1) {
						$answer1 = 'Yes';
					} else {
						$answer1 = 'No';
					}
					$tpl_page .= '<tr><td>'. $line['email'] . '</td><td>'. $line['staffname'] . '</td><td>'. $answer . '</td><td>'. $answer1 . '</td><td>[<a href="?action=email&amp;edit='. $line['id'] . '">'. _gettext('Edit') .'</a>]&nbsp;[<a href="?action=email&amp;del='. $line['id'] . '">'. _gettext('Delete') .'</a>]</td></tr>';
				}
				$tpl_page .= '</table>';
			}
	}
	...
	function backup() {
		global $tc_db, $tpl_page;
		$this->RootOnly();
		$tpl_page .= '<h2>'. _gettext('Backup') . '</h2><br />'. "\n";
		$filename = $_POST['filename'];
		$tpl_page .= '<form action="manage_page.php?action=backup" method="post">
		<input type="text" name="filename">
		<label for="filename">'. _gettext('File name : I.E. mysite.zip') . '</label><br />
		<input type="submit" name="run" value="Submit">';
		if (isset($_POST['run'])) {
			ini_set('max_execution_time', 50000);
			$backup = new ZipArchive();
			$iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator("".KU_ROOTDIR.""));
			if ($backup->open(''.$_POST['filename'].'', ZIPARCHIVE::CREATE) !== TRUE) {
				die ("Could not create Archive");
			}
			foreach ($iterator as $key=>$value) {
				$backup->addFile(realpath($key), $key);
			}
			$backup->close();
			$tpl_page .= _gettext('<br /><br />Site has been saved in the Root Directory as '.$filename.'. Click <a href="'.KU_WEBPATH.'/'.$filename.'">here</a> to download the file.');
		}
	}
	/* Home Page */
	function stat() {
		global $tc_db, $tpl_page;
		$tpl_page .= '<h1><center>'. _gettext('Site Information') .'</center></h1><br />'. "\n";
		$version =  $tc_db->GetOne("SELECT `" . KU_DBPREFIX . "version` FROM `".KU_DBPREFIX."options`");
		$memory = memory_get_usage() / 1024 / 1024;
		$numboard = $tc_db->GetOne('SELECT COUNT(`id`) FROM `'.KU_DBPREFIX.'boards`');
		$totalposts = $tc_db->GetOne('SELECT COUNT(*) FROM `'.KU_DBPREFIX.'posts` WHERE `IS_DELETED` = 0');
		$databasetype = $tc_db->getAttribute(PDO::ATTR_DRIVER_NAME);
		...
		$tpl_page .= '<table cellspacing="2" cellpadding="1" border="1" width="100%"><tr><th>'. _gettext('Anonsaba Version') .'</th><th>'. _gettext('Memory Usage') .'</th><th>'. _gettext('Number of Boards') .'</th><th>'. _gettext('Total Posts') .'</th><th>'. _gettext('Database Type') .'</th></tr>';
		$tpl_page .= "<tr><td>" . $version . "</td><td>" . $memory . " MB</td><td>" . $numboard . "</td><td>" . $totalposts . "</td><td>" . $databasetype ."</td></tr>";
	}
	/* View Announcements */
	...
	function changepwd() {
		...
		<script src="/lib/javascript/chronostrength.js"></script>
		<script type="text/javascript">
		$("input#newpwd").chronoStrength();
		</script>';
	}
	...
	function news() {
		...
		if ($disptable) {
			...
			if (count($results) > 0) {
				$tpl_page .= '<table border="1" width="100%"><tr><th>'. _gettext('Date Added') .'</th><th>'. _gettext('Subject') .'</th><th>'. _gettext('Message') .'</th><th>'. _gettext('Edit/Delete') .'</th></tr>';
				foreach ($results as $line) {
					$tpl_page .= '<tr><td>'. date('F j, Y, g:i a', $line['timestamp']) . '</td><td>'. $line['subject'] . '</td><td>'. $line['message'] . '</td><td>[<a href="?action=news&amp;act=edit&amp;id='. $line['id'] . '">'. _gettext('Edit') .'</a>] [<a href="?action=news&amp;act=del&amp;id='. $line['id'] . '" onclick="return confirm(\'Are you sure you want to delete this news post?\');">'. _gettext('Delete') .'</a>]</td></tr>';
				}
				$tpl_page .= '</table>';
			} else {
	...
	function faq() {
		...
		if ($disptable) {
			...
			if (count($results) > 0) {
				...
				foreach ($results as $line) {
					$tpl_page .= '<tr><td>'. $line['order'] . '</td><td>'. $line['subject'] . '</td><td>'. $line['message'] . '</td><td>[<a href="?action=faq&amp;act=edit&amp;id='. $line['id'] . '">'. _gettext('Edit') .'</a>] [<a href="?action=faq&amp;act=del&id='. $line['id'] . '" onclick="return confirm(\'Are you sure you want to delete this FAQ entry?\');">'. _gettext('Delete') .'</a>]</td></tr>';
				}
				$tpl_page .= '</table>';
			} else {
	...
	function rules() {
		...
		if ($disptable) {
			...
			if (count($results) > 0) {
				$tpl_page .= '<table border="1" width="100%"><tr><th>'. _gettext('Order') .'</th><th>'. _gettext('Heading') .'</th><th>'. _gettext('Message') .'</th><th>'. _gettext('Edit/Delete') .'</th></tr>';
				foreach ($results as $line) {
					$tpl_page .= '<tr><td>'. $line['order'] . '</td><td>'. $line['subject'] . '</td><td>'. $line['message'] . '</td><td>[<a href="?action=rules&amp;act=edit&amp;id='. $line['id'] . '">'. _gettext('Edit') .'</a>] [<a href="?action=rules&amp;act=del&id='. $line['id'] . '" onclick="return confirm(\'Are you sure you want to delete this Rules entry?\');">'. _gettext('Delete') .'</a>]</td></tr>';
				}
				$tpl_page .= '</table>';
			} else {
	...
	function staff() {
		...
		if (isset($_GET['add']) && !empty($_POST['username']) && !empty($_POST['password'])) {
			...
			if (count($results) == 0) {
				if ($_POST['type'] < 3 && $_POST['type'] >= 0) {
							$this->CheckToken($_POST['token']);
							$salt = $this->CreateSalt();
							$tc_db->Execute("INSERT HIGH_PRIORITY INTO `" .KU_DBPREFIX. "staff` ( `username` , `password` , `salt` , `type` , `access`, `addedon` ) VALUES (" .$tc_db->qstr($_POST['username']). " , '" .md5($_POST['password'] . $salt). "' , '" .$salt. "' , '" .$_POST['type']. "' , '" .$_POST['access']. "' , '" .time(). "' )");
							$log = _gettext('Added'). ' ';
					switch ($_POST['type']) {
						case 0:
			...
			if (count($results) > 0) {
				...
				if ($username == KU_ROOT) {
					exitWithErrorPage(_gettext('Access Denied'));
				}
		...
		} elseif (isset($_GET['suspend']) && $_GET['suspend'] > 0) {
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY * FROM `" . KU_DBPREFIX . "staff` WHERE `id` = " . $tc_db->qstr($_GET['suspend']) . "");
			if (count($results) > 0) {
				$username = $results[0]['username'];
				if ($username == KU_ROOT) {
					exitWithErrorPage(_gettext('Access Denied'));
				}
				$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "staff` SET `suspended` = 1 WHERE `id` = " . $tc_db->qstr($_GET['suspend']) . "");
				$tpl_page .= _gettext('Staff successfully suspended');
				management_addlogentry(_gettext('Suspended staff member') . ': ' . $username, 6);
			} else {
				$tpl_page .= _gettext('Invalid staff ID.');
			}
		} elseif (isset($_GET['unsuspend']) && $_GET['unsuspend'] > 0) {
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY * FROM `" . KU_DBPREFIX . "staff` WHERE `id` = " . $tc_db->qstr($_GET['unsuspend']) . "");
			if (count($results) > 0) {
				$username = $results[0]['username'];
				$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "staff` SET `suspended` = 0 WHERE `id` = " . $tc_db->qstr($_GET['unsuspend']) . "");
				$tpl_page .= _gettext('Staff successfully unsuspended');
				management_addlogentry(_gettext('Unsuspended staff member') . ': ' . $username, 6);
			} else {
				$tpl_page .= _gettext('Invalid staff ID.');
			}		
			...
			if (count($results) > 0) {
				if (isset($_POST['submitting'])) {
					...
					$logentry .= ' - '. _gettext('Access Level') . ': '. $_POST['access'];
					$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "staff` SET `boards` = " . $tc_db->qstr(implode('|', $newboards)) . " , `access` = " . $tc_db->qstr($_POST['access']). " , `type` = " .$tc_db->qstr($_POST['type']). " WHERE `id` = " . $tc_db->qstr($_GET['edit']) . "");
				...
				<label for="access">' ._gettext('Access level [<a href="manage_page.php?action=staff&accesshelp=1">?</a>]'). ':</label>
				<select id="access" name="access">';
				$tpl_page .= ($access==1) ? '<option value="1" selected="selected">' ._gettext('1') . '</option>' : '<option value ="1">' ._gettext('1') .'</option>';
				$tpl_page .= ($access==2) ? '<option value="2" selected="selected">' ._gettext('2') . '</option>' : '<option value ="2">' ._gettext('2') .'</option>';
				$tpl_page .= ($access==3) ? '<option value="3" selected="selected">' ._gettext('3') . '</option>' : '<option value ="3">' ._gettext('3') .'</option>';
				$tpl_page .= ($access==4) ? '<option value="4" selected="selected">' ._gettext('4') . '</option>' : '<option value ="4">' ._gettext('4') .'</option>';
				$tpl_page .= ($access==5) ? '<option value="5" selected="selected">' ._gettext('5') . '</option>' : '<option value ="5">' ._gettext('5') .'</option>';
				$tpl_page .= ($access==6) ? '<option value="6" selected="selected">' ._gettext('6') . '</option>' : '<option value ="6">' ._gettext('6') .'</option>';
				$tpl_page .= ($access==7) ? '<option value="7" selected="selected">' ._gettext('7') . '</option>' : '<option value ="7">' ._gettext('7') .'</option>';
				$tpl_page .= ($access==8) ? '<option value="8" selected="selected">' ._gettext('8') . '</option>' : '<option value ="8">' ._gettext('8') .'</option>';
				$tpl_page .= ($access==9) ? '<option value="9" selected="selected">' ._gettext('9') . '</option>' : '<option value ="9">' ._gettext('9') .'</option>';
				$tpl_page .='</select><br />
			...
			<label for="access">' ._gettext('Access level [<a href="manage_page.php?action=staff&accesshelp=1">?</a>]'). ':</label>
			<select id="type" name="access">
			<option value="1">' ._gettext('1').'</option>
			<option value="2">' ._gettext('2').'</option>
			<option value="3">' ._gettext('3').'</option>
			<option value="4">' ._gettext('4').'</option>
			<option value="5">' ._gettext('5').'</option>
			<option value="6">' ._gettext('6').'</option>
			<option value="7">' ._gettext('7').'</option>
			<option value="8">' ._gettext('8').'</option>
			<option value="9">' ._gettext('9').'</option>
			</select><br />
			<label for="type">' ._gettext('Type'). ':</label>
			<select id="type" name="type">
		...
		while($i <= 4) {
			...
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY * FROM `" . KU_DBPREFIX . "staff` WHERE `type` = '" .$numtype. "' ".$access." AND `suspended` != 1 ORDER BY `username` ASC");
			if (count($results) > 0) {
				foreach ($results as $line) {
					...
					if ($line['username'] == KU_ROOT) {
						$tpl_page .= '</td><td></td></tr>';
					} else {
						$tpl_page .= '</td><td>[<a href="?action=staff&edit='. $line['id'] . '">'. _gettext('Edit') . '</a>] &nbsp;[<a href="?action=staff&suspend=' . $line['id'] . '">' . _gettext('Suspend') . '</a>] [<a href="?action=staff&del='. $line['id'] . '">'. _gettext('Delete') .'</a>]</td></tr>'. "\n";
					}
				}
			} else {
		...
		if (count($results) > 0) {
			foreach ($results as $line) {
				...
				if ($line['username'] == KU_ROOT) {
					...
					$tpl_page .= '</td><td>[<a href="?action=staff&edit=' . $line['id'] . '">' . _gettext('Edit') . '</a>]&nbsp;[<a href="?action=staff&unsuspend=' . $line['id'] . '">' . _gettext('Unsuspend') . '</a>] [<a href="?action=staff&del=' . $line['id'] . '">Delete</a>]</td></tr>' . "\n";
				}
			}
		} else {
			$tpl_page .= '<tr><td colspan="5">' . _gettext('None') . '</td></tr>' . "\n";
	...
	function modlog() {
		...
		$tpl_page .= '</table>';
		$tpl_page .= '<p align="right"><a href="manage_page.php?action=modlog&reset=1" onclick="return confirm(\'Are you sure you want to clear the modlog?\');">Clear Modlog</a></p>';
	}
	function proxyban() {
		global $tpl_page;
		$this->AdministratorsOnly();
	...
	function cleanup() {
		...
		if (isset($_POST['run'])) {
			...
			foreach ($results AS $line) {
				if ($line['type'] != 1) {
					$tc_db->Execute("DELETE FROM `" . KU_DBPREFIX . "posts`, `".KU_DBPREFIX."post_files` WHERE `boardid` = " . $line['id'] . " AND `IS_DELETED` = 1 AND `deleted_timestamp` < " . (time() - 604800) . "");
				}
			}
			...
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY * FROM `" . KU_DBPREFIX . "wordfilter` WHERE `word` = " . $tc_db->qstr($_POST['word']) . "");
				if (count($results) == 0) {
					...
					foreach ($_POST['wordfilter'] as $board) {
						...
						if (!empty($check) && !isset($_POST['filterfromall'])) {
							$wordfilter_boards[] = $board;
						}
					}
					if (isset($_POST['filterfromall'])) {
						$query = $tc_db->GetAll("SELECT HIGH_PRIORITY `name` FROM `boards`");
						foreach ($query as $boardlists) {
							$wordfilter_boards[] = $boardlists['name'];
						}
					}
				...
				if (count($results) > 0) {
					if (!isset($_POST['replacedby'])) {
						foreach ($results as $line) {
							...
							$tpl_page .= ' /><br />
							<label for="filterfromall">'. _gettext('All Boards') .'</label>
							<input type="checkbox" name="filterfromall" id="filterfromall" /><br />
	...
	function embeds() {
		...
		if(isset($_GET['act'])) {
			if ($_GET['act'] == 'edit') {
				if (isset($_POST['embeds']) && is_numeric($_POST['width']) && is_numeric($_POST['height'])) {
					$this->CheckToken($_POST['token']);
				...
				if (isset($_POST['embeds']) && isset($_POST['name']) && isset($_POST['filetype']) && isset($_POST['videourl'])) {
					if ($_POST['embeds'] != '') {
						...
						if ($_POST['height'] == '' && $_POST['width'] == '') {
							$_POST['height'] = KU_YOUTUBEHEIGHT;
							$_POST['width'] = KU_YOUTUBEWIDTH;
						}
						$tpl_page .= '<hr />';
						if ($_POST['embeds'] != '' && is_numeric($_POST['width']) && is_numeric($_POST['height'])) {
	...
	function movethread() {
		...
		if (isset($_POST['id']) && isset($_POST['board_from']) && isset($_POST['board_to'])) {
			if (empty($_POST['id'])) {
				exitWithErrorPage(_gettext('Please enter a post ID'));
			}
			$this->CheckToken($_POST['token']);
			...
			if (isset($_POST['mf'])) {
				$files = $tc_db->GetAll("SELECT `file`,`file_type` FROM `" .KU_DBPREFIX. "post_files` WHERE `boardid` = " .$board_from_id. " AND `id` = " .$id);
			...
			$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "post_files` SET `id` = " . $new_id . ", `boardid` = " .$board_to_id. " WHERE `boardid` = " .$board_from_id. " AND `id` = " . $id);
			...
			foreach ($results as $line) {
				if (isset($_POST['mf'])) {
					$files = $tc_db->GetAll("SELECT `file`,`file_type` FROM `" .KU_DBPREFIX. "post_files` WHERE `boardid` = " .$board_from_id. " AND `id` = " .$line['id']);
					foreach ($files as $file) {
						$image = $file['file'];
						$filetype = $file['file_type'];
						$from_pic = KU_BOARDSDIR . $board_from . '/src/'. $image . '.'. $filetype;
				...
				$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "post_files` SET `id` = " . $insert_id . ", `boardid` = " .$board_to_id. " WHERE `boardid` = " .$board_from_id. " AND `id` = " . $line['id']);
	...
	function viewthread() {
		...
		if (!$board) {
			...
			$tpl_page .= '<form method="get" action=""><input type="hidden" name="action" value="viewthread" />'. _gettext('Select Board') .': <select name="board"></form>';
			foreach ($results as $line) {
				$name = $line['name'];
				$id =   $line['id'];
			...
			foreach ($results as $line) {
				...
				if ($posterauthority == "1") {
					$posterauthority = "<font color=\"red\"><strong>## Admin ##<strong></font>";
				} elseif ($posterauthority == "2") {
					$posterauthority = "<font color=\"blue\"><strong>## Mod ##</strong></font>";
				} elseif ($posterauthority == "2" && CheckAccess() >= 9) { 
					$posterauthority = "<font color=\"blue\"><strong>## Super Mod ##</strong></font>";
				} else {
					$posterauthority = "";
				}
				if ($deleted == "1") {
				...
				$tpl_page .= "<table style=\"text-align: left; width: 100%;\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\">
				<tbody>
				<tr>
				...
				$tpl_page .= "<td style=\"vertical-align: top; height: 100%;\">$message</td></tr></tbody></table><br /></form>";
				$first = "0";
			}
			...
			if ($_GET['do'] == 'addfiletype') {
				if (isset($_POST['filetype']) || isset($_POST['image'])) {
					if($_POST['filetype'] ==  'php') {
						exitWithErrorPage(_gettext('That Filetype is not allowed'));
					}
	...
	/* Rebuild just one board */
	function rebuildboard() {
		global $tc_db, $tpl_page;
		$this->AdministratorsOnly();
		$tpl_page .= '<h2>'. _gettext('Rebuild Board') . '</h2><br />';
		if (isset($_POST['board']) && $_POST['board'] != '') {
			$time_start = time();
			$result = $tc_db->GetRow('SELECT HIGH_PRIORITY `id`, `name` FROM `boards` WHERE `name` = '.$tc_db->qstr($_POST['board']).'');
			if ($result['name'] != $_POST['board']) {
				$tpl_page .= 'OH GOD WHAT IS THIS I DON\'T EVEN-<br />';
				return;
			}
			$board_class = new Board($result['name']);
			$board_class->RegenerateAll();
			$tpl_page .= sprintf(_gettext('Regenerated %s'), '/'. $result['name'] . '/') . '<br />';
			unset($board_class);
			flush();
			$tpl_page .= sprintf(_gettext('Rebuild complete. Took <strong>%d</strong> seconds.'), time() - $time_start);
		} else {
			$tpl_page .= '<form action="?action=rebuildboard" method="post">
			<label for="board">'. _gettext('Board') .':</label>' .
			$this->MakeBoardListDropdown('board', $this->BoardList($_SESSION['manageusername'])) .
	...
	function rebuildall() {
		...
		$tpl_page .= sprintf(_gettext('Rebuild took <strong>%g</strong> seconds.<br />'), round(microtime_float() - $time_start, 5));
		...
		if (isset($_GET['updateboard']) && isset($_POST['order']) && isset($_POST['maxpages']) && isset($_POST['maxage']) && isset($_POST['messagelength'])) {
			...
			if ($boardid != '') {
				if ($_POST['order'] >= 0 && $_POST['maxpages'] >= 0 && $_POST['markpage'] >= 0 && $_POST['maxage'] >= 0 && $_POST['messagelength'] >= 0 && ($_POST['defaultstyle'] == '' || in_array($_POST['defaultstyle'], explode(':', KU_STYLES)) || in_array($_POST['defaultstyle'], explode(':', KU_TXTSTYLES)))) {
					...
					$updateboard_imgedit = isset($_POST['imgedit']) ? '1' : '0';
					$updateboard_fileurl = isset($_POST['fileurl']) ? '1' : '0';
					$updateboard_enableemail = isset($_POST['enableemail']) ? '1' : '0';
					$updateboard_enableads = isset($_POST['enableads']) ? '1' : '0';
					...
					if (($_POST['type'] == '0' || $_POST['type'] == '1' || $_POST['type'] == '2' || $_POST['type'] == '3') && ($_POST['uploadtype'] == '0' || $_POST['uploadtype'] == '1' || $_POST['uploadtype'] == '2')) {
						if (!($_POST['uploadtype'] != '0' && $_POST['type'] == '3')) {
							...
							$name = $tc_db->GetOne('SELECT `desc` FROM `boards` WHERE `name` = "'.$_GET['updateboard'].'"');
							$tpl_page .= _gettext('Update successful.');
							$tpl_page .= _gettext('<br /><br /><form action="?action=boardopts" method="post"><input type="hidden" name="board" value="'. $_GET['updateboard'] . '" /><input type="submit" style="border: 1px solid; background: none; text-align: center;" value="'. _gettext('Click to edit '.$name.' options again') .'" /></form>&nbsp;&nbsp;&nbsp;<a href="/manage_page.php?action=boardopts"><input type="submit" style="border: 1px solid; background: none; text-align: center;" value="'. _gettext('Click to edit another boards options') .'" /></a><br />');
			...
			if (count($resultsboard) > 0) {
				...
				$tpl_page .= '<div class="container">
					...
					/* Board Class */
					$tpl_page .= '<label for="boardclass">'. _gettext('Board class') .':</label>
					<select name="boardclass">
					<option value="0"';
					if ($lineboard['boardclass'] == '0') { 
						$tpl_page .= ' selected="selected"'; 
					}
					$tpl_page .= '>'. _gettext('Safe for Work') .'</option>
					<option value="1"';
					if ($lineboard['boardclass'] == '1') { 
						$tpl_page .= ' selected="selected"'; 
					}
					$tpl_page .= '>'. _gettext('Not Safe for Work') .'</option>
					</select>
					<div class="desc">'. _gettext('The class of the board. If it is Safe for Work then only SFW ads will be displayed. If it is Not Safe for Work then only NSFW ads will be displayed.').'</div><br />';
					/* Upload type */
					...
					/* Allow FileURL upload */
					$tpl_page .= '<label for="fileurl">'. _gettext('File URL uploading') .':</label>
					<input type="checkbox" name="fileurl" ';
					if ($lineboard['fileurl'] == '1') {
						$tpl_page .= 'checked ';
					}
					$tpl_page .= ' />
					<div class="desc">'. _gettext('Allow posting an image with File URL.') .' '. _gettext('Default') .': <strong>'. _gettext('Yes') .'</strong></div><br />';
					/* Allowed filetypes */
					...
					$tpl_page .= '<label for="max_files">'. _gettext('Maximum files per post') .':</label>
					<input type="text" name="max_files" value="'.$lineboard['max_files'].'" />
					<div class="desc">'. _gettext('Maximum number of files to be posted to a single post.') . ' '. _gettext('Default') .': <strong>1</strong></div><br />';
					/* Maximum image size */
					...
					/* Enable Email */
					$tpl_page .= '<label for="enableemail">'. _gettext('Enable Email') .':</label>
					<input type="checkbox" name="enableemail" ';
					if ($lineboard['enableemail'] == '1') {
						$tpl_page .= 'checked ';
					}
					$tpl_page .= ' />
					<div class="desc">'. _gettext('Allow posting with Email.') .' '. _gettext('Default') .': <strong>'. _gettext('Yes') .'</strong></div><br />';
					/* Enable Ads */
					$tpl_page .= '<label for="enableads">'. _gettext('Enable Ads') .':</label>
					<input type="checkbox" name="enableads" ';
					if ($lineboard['enableads'] == '1') {
						$tpl_page .= 'checked ';
					}
					$tpl_page .= ' />
					<div class="desc">'. _gettext('Allow ads on this board.') .' '. _gettext('Default') .': <strong>'. _gettext('Yes') .'</strong></div><br />';
					/* Image edit */
					$tpl_page .= '<label for="imgedit">'. _gettext('Enable Image editing') .':</label>
					<input type="checkbox" name="imgedit" ';
					if ($lineboard['imgedit'] == '1') {
						$tpl_page .= 'checked ';
					}
					$tpl_page .= ' />
					<div class="desc">'. _gettext('Allow image editing.') .' '. _gettext('Default') .': <strong>'. _gettext('No') .'</strong></div><br />';
					/* Show ID */
				...
				if (count($results) > 0) {
					...
					if ($result > 0) {
						if ($_GET['unsticky'] == 1) {
							...
							$tpl_page .= _gettext('Thread successfully unstickied.');
	...
	function stickyforms() {
		...
		foreach ($results_boards as $line_board) {
			...
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY `id`, `stickied` FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $line_board['id'] . " AND `IS_DELETED` = '0' AND `parentid` = '0' AND `stickied` = '1'");
			if (count($results) > 0) {
				foreach ($results as $line) {
					$output .= '<a href="?action=stickypost&board='. $line_board['name'] . '&postid='. $line['id'] . '&unsticky='.$line['stickied'].'">#'. $line['id'] . '</a><br />';
				}
			} else {
				...
				if (count($results) > 0) {
					...
					if (count($results) > 0) {
						if ($_GET['unlock'] == 1) {
							...
							management_addlogentry(_gettext('Unlocked thread') . ' #'. intval($_GET['postid']) . ' - /'. $_GET['board'] . '/', 5);
						} else {
							...
							management_addlogentry(_gettext('Locked thread') . ' #'. intval($_GET['postid']) . ' - /'. $_GET['board'] . '/', 5);
						}
	...
	function lockforms() {
		...
		foreach ($results_boards as $line_board) {
			$output .= '<h2>/'. $line_board['name'] . '/</h2>';
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY `id`, `locked` FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $line_board['id'] . " AND `IS_DELETED` = '0' AND `parentid` = '0' AND `locked` = '1'");
			if (count($results) > 0) {
				foreach ($results as $line) {
					$output .= '<a href="?action=lockpost&board='. $line_board['name'] . '&postid='. $line['id'] . '&unlock='.$line['locked'].'">#'. $line['id'] . '</a><br />';
				}
			} else {
	...
	function delposts($multidel=false) {
		...
		if (isset($_POST['boarddir']) || isset($_GET['boarddir'])) {
			if (!isset($_GET['boarddir']) && isset($_POST['boarddir'])) {
				...
				if (isset($_POST['delthreadid'])) {
					if ($_POST['delthreadid'] > 0) {
						...
						if (count($results) > 0) {
							if (isset($_POST['fileonly'])) {
								$result = $tc_db->GetAll("SELECT HIGH_PRIORITY * FROM `" . KU_DBPREFIX . "post_files` WHERE `boardid` = " . $board_id . " AND `IS_DELETED` = '0' AND `id` = " . $tc_db->qstr($_POST['delthreadid']) . "");
								foreach ($result as $line) {
									if (!empty($line['file'])) {
										...
										if ($del) {
											...
											$tc_db->Execute("UPDATE `".KU_DBPREFIX."post_files` SET `file` = 'removed', `file_md5` = '', `IS_DELETED` = 1 WHERE `boardid` = " . $board_id . " AND `id` = ".$_POST['delthreadid']." LIMIT 1");
				...
				} elseif (isset($_POST['delpostid']) && $_POST['delpostid'] != '') {
					...
					if (count($results) > 0) {
						foreach ($results as $line) {
							$delpost_id = $line['id'];
							$delpost_parentid_fetch = $tc_db->GetAll("SELECT HIGH_PRIORITY `parentid` FROM `" . KU_DBPREFIX . "posts` WHERE `boardid` = " . $board_id . " AND `IS_DELETED` = '0' AND `id` = ".$line['id']."");
							$delpost_parentid = $delpost_parentid_fetch[0]['parentid'];
						}
						if (isset($_POST['fileonly'])) {
			...
			foreach ($resultsreport as $linereport) {
				...
				foreach($results as $line) {
					if ($line['IS_DELETED'] == 0) {
						...
						$files = $tc_db->GetAll('SELECT * FROM `' . KU_DBPREFIX . 'post_files` WHERE `boardid` = ' . $reportboardid . ' AND `id` = ' . $tc_db->qstr($linereport['postid']));
						if (!$files) {
							$tpl_page .= 'none';
						}
						foreach ($files as $lines) {
							if ($lines['file'] == 'removed') {
								...
								$tpl_page .= '<a href="'. KU_BOARDSPATH . '/'. $linereport['board'] . '/src/'. $lines['file'] . '.'. $lines['file_type'] . '"><img src="'. KU_BOARDSPATH . '/'. $linereport['board'] . '/thumb/'. $lines['file'] . 's.'. $lines['file_type'] . '" border="0"></a>';
							} else {
								$tpl_page .= '<a href="'. KU_BOARDSPATH . '/'. $linereport['board'] . '/src/'. $lines['file'] . '.'. $lines['file_type'] . '">File</a>';
							}
						}
						$tpl_page .= '</td><td>';
						if ($line['message'] != '') {
	...
	function bans() {
		...
		if (!$_POST['bancheckip'] && $_POST['filecheck'] &&  $_POST['addban']) {
			$banhash = $_POST['hash'];
			$tc_db->Execute("INSERT INTO `".KU_DBPREFIX."bannedhashes` ( `md5` , `bantime` , `description`, `addedby`, `dateadded` ) VALUES ( ".$tc_db->qstr($banhash)." , ".$tc_db->qstr($_POST['banhashtime'])." , ".$tc_db->qstr($_POST['banhashdesc'])." , '".$_SESSION['manageusername']."' , " .time(). ")");
			management_addlogentry('Banned md5 hash '. $banhash . ' with a description of '. $_POST['banhashdesc'], 8);
			$tpl_page .= _gettext('Hash ban successfully placed.')."<br />";
		}
		if (isset($_GET['banboard']) && isset($_GET['banpost'])) {
			...
			if (!empty($ban_board_id)) {
				...
				$querys = $tc_db->GetAll('SELECT HIGH_PRIORITY * FROM `' . KU_DBPREFIX .'post_files` WHERE `boardid` = "' . $ban_board_id. '" AND `id` = ' .$tc_db->qstr($_GET['banpost']) .'');
				if (count($results) > 0 || count($querys) > 0) {
					$ban_ip = md5_decrypt($results[0]['ip'], KU_RANDOMSEED);
					$ban_hash = $querys[0]['file_md5'];
		...
		if (((isset($_POST['ip']) || isset($_POST['multiban'])) && isset($_POST['seconds']) && $_POST['checkbanip'] && (!empty($_POST['ip']) || (empty($_POST['ip']) && !empty($_POST['multiban'])))) || $instantban) {
			if ($_POST['seconds'] >= 0 || $instantban) {
				...
				if (isset($_POST['banhashtime']) && $_POST['filecheck'] && $_POST['banhashtime'] !== '' && ($_POST['hash'] !== '' || isset($_POST['multibanhashes'])) && $_POST['banhashtime'] >= 0) {
		...
		$tpl_page .= '<br />
		<label for="checkbanip">'._gettext('Ban IP?').':</label>
		<input type="checkbox" name="checkbanip" id="checkbanip" checked="checked" />
		<div class="desc">'._gettext('If you want to ban the IP make sure this is checked.').'</div><br />
		...
		<label for="filecheck">'._gettext('Ban File Hash?').':</label>
		<input type="checkbox" name="filecheck" id="filecheck" />
		<div class="desc">'. _gettext('Check for yes.') . '</div><br />
		<label for="hash">'._gettext('File Hash Code').':</label>
		<input type="text" name="hash" value="'. $ban_hash . '" />
		<div class="desc">'. _gettext('The hash of the File you are trying to ban. <b>IF YOU CHECK BANNING THE FILE DO NOT LEAVE THIS BLANK.</b>') . '</div><br />
		...
		<div class="desc">'. _gettext('Presets') .':&nbsp;<a href="#" onclick="document.banform.reason.value=\''. _gettext('Child Pornography') .'\';return false;">CP</a>&nbsp;<a href="#" onclick="document.banform.reason.value=\''. _gettext('Proxy') .'\';return false;">'. _gettext('Proxy') .'</a>&nbsp;<a href="#" onclick="document.banform.reason.value=\''. _gettext('Spam') .'\';return false;">'. _gettext('Spam') .'</a></div><br />
		...
		$tpl_page .= '</fieldset>
		<input type="submit" name ="addban" value="'. _gettext('Add ban') . '" /><img src="clear.gif" />
		</form>
		<hr /><br />';
		for ($i = 2; $i >= 0; $i--) {
			switch ($i) {
			...
			if (count($results) > 0) {
				...
				$tpl_page .= '</th><th>'. _gettext('Boards') . '</th><th>'. _gettext('Reason') . '</th><th>'. _gettext('Staff Note') . '</th><th>'. _gettext('Date added') . '</th><th>'. _gettext('Expires') . '</th><th>'. _gettext('Added By') . '</th><th>&nbsp;</th></tr>';
				foreach ($results as $line) {
		...
		$tpl_page .= '<br /><br /><strong>'. _gettext('File hash bans') . ':</strong><br /><table border="1" width="100%"><tr><th>'. _gettext('Hash') . '</th><th>'. _gettext('Description') . '</th><th>'. _gettext('Date added') . '</th><th>'. _gettext('Expires') . '</th><th>'. _gettext('Added by') . '</th><th>&nbsp;</th></tr>';
		if ($_GET['hashbans'] == 1) {
			$hashlimit = '';
		} elseif ($_GET['hashbans'] == 2) {
			$hashlimit = 'LIMIT 100';
		} else {
			$hashlimit = 'LIMIT 15';
		}
		$hiddenhashban = $tc_db->GetAll('SELECT HIGH_PRIORITY COUNT(*) FROM `' . KU_DBPREFIX . 'bannedhashes`');
		if ($hiddenhashban[0][0] < 15) {
			$hiddenhashbans = 0;
		} else {
			$hiddenhashbans = $hiddenhashban[0][0] - 15;
		}
		$results = $tc_db->GetAll('SELECT HIGH_PRIORITY * FROM `'.KU_DBPREFIX.'bannedhashes` ORDER BY `id` DESC '.$hashlimit.'');
		if (count($results) == 0) {
			$tpl_page .= '<tr><td colspan="6">'. _gettext('None') . '</td></tr>';
		} else {
			foreach ($results as $line) {
				$tpl_page .= '<tr><td>'. $line['md5'] . '</td><td>'. $line['description'] . '</td><td>'. date("F j, Y, g:i a", $line['dateadded']).'</td><td>';
				$bantimedisplay = $line['dateadded'] + $line['bantime'];
				if ($line['bantime'] == 0) {
					$tpl_page .= '<strong>'. _gettext('Does not expire') . '</strong>';
				} else {
					$tpl_page .= date("F j, Y, g:i a", $bantimedisplay);
				}
				$tpl_page .= '</td><td>'.$line['addedby'].'</td><td>';
				$tpl_page .= '[<a href="?action=bans&delhashid='. $line['id'] . '">Delete</a>]</td></tr>';
			}
		}
		$tpl_page .= '</table>';
		$display = $tc_db->GetAll('SELECT COUNT(*) FROM `'.KU_DBPREFIX.'bannedhashes`');
		foreach ($display as $stuff) {
			$displaystuff = $stuff['COUNT(*)'];
		}
		if ($displaystuff != 0) {
			$tpl_page .= sprintf(_gettext('%s Hash bans not shown. '), $hiddenhashbans);
			$tpl_page .= '<a href="?action=bans&hashbans=1">View all Hash bans</a>&nbsp;<a href="?action=bans&hashbans=2">View last 100 Hash bans</a>';
		} else {
			$tpl_page .= '';
		}
	}
	...
	/* Lock/Unlock All boards */
	function lockunlock() {
		global $tpl_page, $tc_db;
		$this->ModeratorsOnly();
		if($this->CheckAccess() < 9) {
			exitWithErrorPage('You do not have permission to access this page');
		}
		if (isset($_POST['lock'])) {
			$tpl_page .= '<hr />';
			$tc_db->Execute('UPDATE `'.KU_DBPREFIX.'boards` SET `locked` = 1');
			management_addlogentry(_gettext('Locked all boards'), 0);
			$tpl_page .= '<h3>All boards have been locked.</h3><br />';
		}
		if (isset($_POST['unlock'])) {
			$tpl_page .= '<hr />';
			$tc_db->Execute('UPDATE `'.KU_DBPREFIX.'boards` SET `locked` = 0');
			management_addlogentry(_gettext('Unlocked all boards'), 0);
			$tpl_page .= '<h3>All boards have been unlocked.</h3><br />';
		}
		$tpl_page .= '<form action="?action=lockunlock" method="post">
					<h2>Lock/Unlock all Boards</h2>
					<br /><input type="submit" name="lock" id="lock" value="Lock">
					&nbsp;&nbsp;
	...
	function delunusedimages($verbose = false) {
		...
		foreach ($resultsboard as $lineboard) {
			...
			$file_md5list = array();
			$results = $tc_db->GetAll("SELECT HIGH_PRIORITY `file_md5` FROM `" . KU_DBPREFIX . "post_files` WHERE `boardid` = " . $lineboard['id'] . " AND `IS_DELETED` = 0 AND `file` != '' AND `file` != 'removed' AND `file_md5` != ''");
			foreach ($results as $line) {
