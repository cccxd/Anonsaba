...
if (!$manage_class->ValidateSession(true)) {
	...
	if ($_SESSION['manageusername'] == 'root') {
		...
		if($_SESSION['managepassword'] == md5('root'.$salt))
	...
	if ($update != $version) {
		$tpl_links .= '<br /><strong><font color="red">' . _gettext('NOTICE: NEW UPDATES AVAILABLE!') . '</font></strong>';
	}
	$tpl_links .= '<br />' . _gettext('Staff rights') . ': <strong>';
	if ($manage_class->CurrentUserIsAdministrator() && $_SESSION['manageusername'] == KU_ROOT) {
		$tpl_links .= _gettext('Root Administrator');
	} elseif ($manage_class->CurrentUserIsAdministrator()) {
		$tpl_links .= _gettext('Administrator');
	} elseif ($manage_class->CurrentUserIsModerator() && $manage_class->CheckAccess() >= 9) {
		$tpl_links .= _gettext('Super Moderator');
	...
	$pms = $tc_db->GetOne("SELECT HIGH_PRIORITY COUNT(*) FROM `" . KU_DBPREFIX . "pms` WHERE `read` != '1' AND `to` = '".$_SESSION['manageusername']."'");
	$tpl_links .= "</strong>";
	$tpl_links .= '<li><a href="' . KU_WEBFOLDER . '" target="_top">' . _gettext('Home') . '</a></li>' . "\n";
	$tpl_links .= '<li><a href="manage_page.php?action=logout">'._gettext('Log out').'</a></li>' . "\n";
	$tpl_links .= '<li><a href="manage_page.php?action=pms">'._gettext('Private Messages') . ' [' . $pms . ']</a></li>
	...
	'<ul>' ."\n" .
	'<li><a href="manage_page.php?action=stat">'._gettext('Site Information').'</a></li>
	<li><a href="manage_page.php?action=announcements">'._gettext('View Announcements').'</a></li>
	...
	if ($manage_class->CurrentUserIsAdministrator()) {
		...
		'<ul>' . "\n" .
		'<li><a href="manage_page.php?action=email">' . _gettext('Email post and reports') . '</a></li>' . "\n" .
		'<li><a href="manage_page.php?action=addannouncement">' . _gettext('Announcements') . '</a></li>' . "\n" .
		'<li><a href="manage_page.php?action=checkversion">' . _gettext('Check for updates') . '</a></li>' . "\n" .
		...
		<!--<li><a href="manage_page.php?action=suspended">' . _gettext('Suspended') . '</a></li>-->
		...
		<li><a href="manage_page.php?action=rebuildboard">' . _gettext('Rebuild board') . '</a></li>
	...
	if ($manage_class->CurrentUserIsAdministrator() || $manage_class->CurrentUserIsModerator()) {
		$open_reports = $tc_db->GetOne("SELECT HIGH_PRIORITY COUNT(*) FROM `" . KU_DBPREFIX . "reports` WHERE `cleared` = '0'");
		$tpl_links .= section_html(_gettext('Moderation'), 'moderation') .
		'<ul>
		<li><a href="manage_page.php?action=reports">' . _gettext('View Reports') . ' [' . $open_reports . ']</a></li>
